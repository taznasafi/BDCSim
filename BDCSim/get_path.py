...
class pathFinder:
    ...
    def fc_2_df(cls, fc):
        ...
        not_include_fields = ['Shape', 'Shape_Length', 'Shape_Area']
        ...
        for field in field_names:
            with arcpy.da.SearchCursor(fc, field_names=field) as c:
                for row in c:
                    fc_dict[field].append(row[0])

        return pd.DataFrame.from_dict(fc_dict)
